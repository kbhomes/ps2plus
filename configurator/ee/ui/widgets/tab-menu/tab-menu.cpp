...
bool PS2Plus::UI::BeginTabMenu(const char *key) {
...
bool PS2Plus::UI::BeginTabMenuItem(const char *label) {
...
void PS2Plus::UI::EndTabMenuItem() {
    ImGui::PopStyleVar(/* ImGuiStyleVar_FramePadding */);
    ImGui::EndTabItem();
}


void PS2Plus::UI::TabMenu(const char *key, std::function<void()> code) {
    if (PS2Plus::UI::BeginTabMenu(key)) {
        code();
        PS2Plus::UI::EndTabMenu();
    }
}

void PS2Plus::UI::TabMenuItem(const char *label, std::function<void()> code) {
    if (PS2Plus::UI::BeginTabMenuItem(label)) {
        code();
        PS2Plus::UI::EndTabMenuItem();
    }
}