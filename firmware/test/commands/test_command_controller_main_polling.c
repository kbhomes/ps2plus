...
void test_command_controller_main_polling_digital_mode() {
  ...
  polling_check_digital_buttons(MP_COMMAND_ID, MP_PAYLOAD_BYTES);
}

void test_command_controller_main_polling_analog_mode() {
  state.analog_mode = CMAnalog;
  polling_check_digital_buttons(MP_COMMAND_ID, MP_PAYLOAD_BYTES);
  polling_check_analog_joysticks(MP_COMMAND_ID, MP_PAYLOAD_BYTES);
}
...
void test_command_controller_main_polling_analog_full_mode() {
  ...
  polling_check_digital_buttons(MP_COMMAND_ID, MP_PAYLOAD_BYTES);
  polling_check_analog_joysticks(MP_COMMAND_ID, MP_PAYLOAD_BYTES);
  polling_check_analog_pressures(MP_COMMAND_ID, MP_PAYLOAD_BYTES);
}
...
void test_command_controller_main_polling_with_motors() {
  ...
  polling_check_digital_buttons(MP_COMMAND_ID, MP_PAYLOAD_MOTOR_CONTROL_BYTES);
  polling_check_analog_joysticks(MP_COMMAND_ID, MP_PAYLOAD_MOTOR_CONTROL_BYTES);
  polling_check_analog_pressures(MP_COMMAND_ID, MP_PAYLOAD_MOTOR_CONTROL_BYTES);
