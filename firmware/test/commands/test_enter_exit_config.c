...
void test_enter_exit_config_command_digital_mode_polling() {
  ...
  polling_check_digital_buttons(&state, EEC_COMMAND_ID, EEC_PAYLOAD_EXIT_CONFIG_BYTES);
}

void test_enter_exit_config_command_analog_mode_polling() {
  state.analog_mode = CMAnalog;
  polling_check_digital_buttons(&state, EEC_COMMAND_ID, EEC_PAYLOAD_EXIT_CONFIG_BYTES);
  polling_check_analog_joysticks(&state, EEC_COMMAND_ID, EEC_PAYLOAD_EXIT_CONFIG_BYTES);
}
...
void test_enter_exit_config_command_analog_full_mode_polling() {
  ...
  polling_check_digital_buttons(&state, EEC_COMMAND_ID, EEC_PAYLOAD_EXIT_CONFIG_BYTES);
  polling_check_analog_joysticks(&state, EEC_COMMAND_ID, EEC_PAYLOAD_EXIT_CONFIG_BYTES);
  polling_check_analog_pressures(&state, EEC_COMMAND_ID, EEC_PAYLOAD_EXIT_CONFIG_BYTES);
}

void test_enter_exit_config_command_enter_config() {
  state.config_mode = false;

  helper_run_command(&state, EEC_COMMAND_ID, EEC_PAYLOAD_ENTER_CONFIG_BYTES, polling_payload_size_for_mode(state.config_mode, state.analog_mode));
...
void test_enter_exit_config_command_exit_config() {
  ...
  helper_run_command(&state, EEC_COMMAND_ID, EEC_PAYLOAD_EXIT_CONFIG_BYTES, polling_payload_size_for_mode(state.config_mode, state.analog_mode));
