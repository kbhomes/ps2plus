...
#define PS2PLUS_SC_COMMAND_ID 0x72

void test_command_ps2plus_set_configuration_joystick_axis_range_remapping() {
  // helper_enable_debug();
  ...
  for (controller_joystick_axis_range_remapping jsar = 0; jsar < NUM_JOYSTICK_AXIS_RANGES; jsar++) {
    // Set the specific joystick axis range remapping value
    uint8_t command_input[] = { jsar & 0xFF, (jsar >> 8) & 0xFF, 0x02, axis_mappings[jsar] };
...
void test_command_ps2plus_set_configuration_joystick_digital_mode_enabled() {
  // helper_enable_debug();
  bool values[] = { false, true };

  for (size_t i = 0; i < sizeof(values); i++) {
    uint8_t command_input[] = { 0x0C, 0x00, 0x01, (values[i] ? 0xFF : 0x00) };
    uint8_t expected_output[] = { 0x00, 0x00, 0x00, 0x00 };
    uint8_t *actual_output = helper_run_command(&state, PS2PLUS_SC_COMMAND_ID, command_input, sizeof(command_input));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_output, actual_output, sizeof(command_input));
...
void test_command_ps2plus_set_configuration_global_button_remapping_enabled() {
  ...
  for (size_t i = 0; i < sizeof(values); i++) {
    uint8_t command_input[] = { 0x0D, 0x00, 0x01, (values[i] ? 0xFF : 0x00) };
    uint8_t expected_output[] = { 0x00, 0x00, 0x00, 0x00 };
    uint8_t *actual_output = helper_run_command(&state, PS2PLUS_SC_COMMAND_ID, command_input, sizeof(command_input));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_output, actual_output, sizeof(command_input));
...
void test_command_ps2plus_set_configuration_unknown() {
  ...
  uint8_t command_input[] = { id & 0xFF, (id >> 8) & 0xFF, 0x02, 0x5A };
